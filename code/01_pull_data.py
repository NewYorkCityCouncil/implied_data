...
if dept == "SocialServices":  
    ...
    if sum(matches) == 0: 
        dept_opendata = "No Open Data found."
    else: 
        full_dept = agency_names[matches][0]
        print(full_dept)

        # keep only relevant data
        dept_opendata = datasets[datasets.datasetinformation_agency == full_dept][['name',  'type']].to_string()
...
if dept == "SocialServices":  
    ...
    if sum(matches) == 0: 
        ...
        dept_mmr = dept_mmr[['mmr_indicator_name', 'mmr_indicator_description', 'mmr_indicator_source']].to_string()
